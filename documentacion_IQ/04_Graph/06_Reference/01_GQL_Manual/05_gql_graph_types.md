...
Nodes can have multiple labels to support inheritance and categorization. You can specify multiple labels for a node type, but one label (the "key label") _must_ uniquely identify the node type
...
- Defines targeted nodes using a simple _constraint pattern_ that specifies which nodes the constraint applies to.
...
This syntax creates a node key constraint named person_pk for all nodes with _at least_ the Person label. The constraint ensures that each node in the graph gets uniquely identified by its id property. No two nodes with the Person label can have the same id value.
...
- Optimize GQL query performance for graph
