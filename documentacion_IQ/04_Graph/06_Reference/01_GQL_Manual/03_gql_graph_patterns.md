...
This pattern matches all nodes that have **both** the Place and City labels (indicated by the & operator) and whose name property equals "New York". This combination of required labels and properties is called the _filler_ of the node pattern.
...
A&B
Labels need to include both A and B.

A\|B
Labels need to include at least one of A or B.

!A
Labels need to exclude A.
...
See Bind variable-length pattern edge variables.
...
The pattern finds a person along with their education, employment, and content preferences all at onceâa comprehensive profile query.
...
This pattern matches pairs of different persons that know each other _but also_
...
For more information, see horizontal aggregation.
