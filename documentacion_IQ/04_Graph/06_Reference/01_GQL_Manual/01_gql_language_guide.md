...
- Write common GQL queries â neighbors, multi-hop traversal, shared connections, and entity existence checks

- Filter and aggregate graph data â FILTER, WHERE, GROUP BY, and aggregate functions

- Write graph pattern queries â multi-hop patterns, path modes, variable reuse, and OPTIONAL MATCH

- Optimize GQL query performance â filtering strategy, traversal limits, and key constraint recommendations
...
You can specify graph models by using GQL standard syntax. In this case, they're called graph types.
...
For more information, see "The LDBC Social Network Benchmark".
...
- **MATCH** finds all nodes labeled Person.

- **RETURN** shows their first and last names.
...
- **MATCH**â¯finds all nodes (p) labeledâ¯Person.

- **FILTER** nodes (p) whose first name is Alice.

- **RETURN**â¯shows their first, last name, and birthday.
...
- **MATCH** finds all pairs of Person nodes that know each other.

- **FILTER** keeps only the pairs where both people have the same birthday.

- **RETURN** counts how many such friend pairs exist.
...
- **MATCH**: Identifies the graph pattern to search forâthis is where you define the structure of the data you're interested in.

- **LET**: Assigns new variables or computed values based on matched dataâadds derived columns to the result.

- **FILTER**: Narrows down the results by applying conditionsâremoves rows that donât meet the criteria.

- **ORDER BY**: Sorts the filtered dataâhelps organize the output based on one or more fields.

- **OFFSET** and **LIMIT**: Restrict the number of rows returnedâuseful for pagination or top-k queries.

- **RETURN**: Specifies the final outputâdefines what data should be included in the result set and performs aggregation.
...
`
-- Data flows: Match â Let â Filter â Order â Limit â Return
...
- **MATCH** finds people who work at companies with "Air" in their name.

- **LET** creates full names by combining first and family names.

- **FILTER** keeps only Contoso employees.

- **ORDER BY** sorts by full name.

- **LIMIT** takes the first 10 results.

- **RETURN** returns names and company locations.
...
When you run a query, you get back an _execution outcome_ that consists of:
...
Success with at least one row

00001
note: successful completion - omitted result
Success with no table (currently unused)
...
View complete GQLSTATUS codes reference
...
WALK
Allows repeated nodes and edges. No restrictions on traversal.

TRAIL
No repeated edges, but nodes may repeat. This is the default path mode.

SIMPLE
No repeated nodes (except the first and last may be equal).

ACYCLIC
No repeated nodes at all, preventing any cycle in the path.
...
Learn comprehensive pattern syntax and advanced techniques
...
Graph supports basic and full linear statement composition (chaining MATCH, LET, FILTER, RETURN, and other core statements). Set operations like UNION DISTINCT, EXCEPT, INTERSECT, and OTHERWISE aren't yet supported. For more information, see the article on current limitations.
...
Only the _immediately_ following statement can see the sort order that ORDER BY establishes.
...
Learn comprehensive type system details
...
Learn comprehensive expression syntax and all built-in functions
...
Graph supports basic and full linear statement composition. Set operations like UNION DISTINCT, EXCEPT, INTERSECT, and OTHERWISE aren't yet supported. For more information, see the article on current limitations.
...
FILTER fullName CONTAINS 'Smith'                    -- Filter for fullNames with âSmithâ substring (p is still bound)
...
As explained in the section on query results,
GQL reports rich status information related to the success or potential failure of execution.
See the GQL status codes reference for the complete list.
...
- **Try the quickstart** - Follow the hands-on quickstart and tutorial to get practical experience

- **Practice basic queries** - Try the examples from this guide with your own data

- **Learn graph patterns** - Explore the comprehensive pattern syntax

- **Explore data types** - Understand GQL values and value types

**For experienced users:**

- **Advanced expressions** - Learn GQL expressions and functions
...
Start with the quickstart tutorial if you prefer learning by doing, or dive into graph patterns if you want to learn the query language first.

## Related content

**How-to guides:**

- Write common GQL queries - Practical query patterns for neighbors, multi-hop, shared connections, and more.

- Filter and aggregate graph data - FILTER, WHERE, GROUP BY, and aggregate functions.

- Write graph pattern queries - Multi-hop traversal, path modes, variable reuse, and optional matching.

- Optimize GQL query performance - Best practices for writing efficient GQL queries.
