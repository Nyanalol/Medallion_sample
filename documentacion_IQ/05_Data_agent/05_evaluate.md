...
# Evaluate your data agent (preview)
			...
			Feedback
				...
				In this article
					...
					Evaluation with the Fabric SDK allows you to programmatically test how well your Data Agent responds to natural language questions. Using a simple Python interface, you can define ground truth examples, run evaluations, and analyze resultsâall within your notebook environment. This helps you validate accuracy, debug errors, and confidently improve your agent before deploying it to production.
...
The next step is to run the evaluation using the evaluate_data_agent function. This function compares the agentâs responses against your expected results and stores the evaluation metrics.
...
After running the evaluation, you can retrieve a high-level summary of the results using the get_evaluation_summary function. This function provides insights into how well your Data Agent performed overall â including metrics like how many responses matched the expected answers.
...
The returned DataFrame includes aggregated metrics such as the number of correct, incorrect, or unclear responses. This result helps you quickly assess the agentâs accuracy and identify areas for improvement.
...
- table_name _(str, optional)_ â The name of the table containing the evaluation summary results. Defaults to 'evaluation_output'.

- verbose _(bool, optional)_ â If set to True, prints a summary of evaluation metrics to the console.  Defaults to False.

**Returns:**

DataFrame â A pandas DataFrame containing summary statistics for the evaluation, such as:
...
- evaluation_id_(str)_ â Required. The unique identifier for the evaluation run to retrieve details for.

- table_name_(str, optional)_ â The name of the table containing the evaluation results. Defaults to evaluation_output.

- get_all_rows_(bool, optional)_ â Whether to return all rows from the evaluation (True) or only rows where the agentâs response was incorrect or unclear (False). Defaults to False.

- verbose_(bool, optional)_ â If set to True, prints a summary of evaluation metrics to the console. Defaults to False.

**Returns:**

DataFrame â A pandas DataFrame containing row-level evaluation results, including:
...
By default, the Fabric SDK uses a built-in prompt to evaluate whether the Data Agentâs actual answer matches the expected answer. However, you can supply your own prompt for more nuanced or domain-specific evaluations using the critic_prompt parameter.
...
The **Diagnostics** button allows you to **download a full snapshot of your Data Agentâs configuration and execution steps**. This export includes details such as data source settings, applied instructions, example queries used, and the underlying steps the Data Agent took to generate its response.

Use this feature when working with Microsoft Support or troubleshooting unexpected behavior. By reviewing the downloaded file, you can see exactly how the Data Agent processed your request, which configurations were applied, and where potential issues may have occurred. This level of transparency makes it easier to debug and optimize your Data Agentâs performance.

## Next steps

- Use the Fabric data agent SDK

- Access sample notebooks on how to use the data agent SDK
